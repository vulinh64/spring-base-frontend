...
function PostEditPageInner() {
  ...
  const { data: existingPost, isLoading: loadingPost } = usePost(slugParam!);
  const { data: categories } = useCategories({ size: 100 });

  const draftKey = `post-edit:${slugParam}`;
  ...
  useEffect(() => {
    if (existingPost && !restoredFromDraft) {
      ...
      setTagsInput(
        existingPost.tags?.map((t) => t.displayName).join(", ") ?? ""
      );
    }
  }, [existingPost, restoredFromDraft]);
  ...
  const editMutation = useMutation({
    mutationFn: (request: PostCreationRequest) =>
      postApi.edit(existingPost!.id, request),
    onSuccess: () => {
      discardDraft();
      queryClient.invalidateQueries({ queryKey: ["posts"] });
      queryClient.invalidateQueries({ queryKey: ["post", slugParam] });
      toast.success("Post updated.");
      router.push(`/post/${slugParam}`);
    },
  ...
  return (
    <div>
      <h1 className="text-2xl font-bold text-gray-100 mb-6">Edit Post</h1>
      {error && (
        <div className="mb-4">
